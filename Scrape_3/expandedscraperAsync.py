...
class Story(object):
    ...
    async def get_meta(self, sess=None):
    ...
    #irelevant
    async def get_work(self, sess=None):
        self.sess = sess
        self.error = 200
        try:
            await self.work.make_request(sess)
    ...
    async def download_chapters(self):
        ...
        await self.download_comments()

    async def download_comments(self):
        ...
        if a_tag != None:
            while a_tag.a != None:
                ...
                req = await self.sess.request(method='GET', url=link)
                new_html = await req.text()
