...
from expandedscraperAsync import Chapter, Story, ScraperPage
...
_datapath = "../../data/scrape3_testing/{}"
sleep_time = 60
total_sleep = 0
totaltime = 0
checkpoint = {}
...
@handle_io
def get_bkmrkpg(upath, idx):
    url = "https://archiveofourown.org{0}bookmarks?page={1}"
    return ScraperPage(url.format(upath, idx), search_type='bookmark')

def get_bookmarks(upath):
    bookmarks = []
    #get first page:
    for attempt in range(5):
        start = time.perf_counter()
        page1 = get_bkmrkpg(upath, 1)
        break
    else:
        sleep_time = 60
        return bookmarks
...
def get_giftpg(upath, idx):
    url = "https://archiveofourown.org{0}gifts?page={1}"
    return ScraperPage(url.format(upath, idx), search_type='work')
...
if __name__ == "__main__":
    ...
    try:
        ...
        with open(_datapath.format('fulllist.json')) as fp:
            userlist = json.load(fp)

        workmeta_writer = csv.DictWriter(workmeta_pointer, 
                                        fieldnames = ['ident', 'title', 'author', 'rating', 'warnings', 'category' 'fandoms', 'relationship', 'characters', 'additional_tags',
        ...
        for upath in userlist[:1000]:
            ...
            workchunks = list(chunks([Story(ident) for ident in work_idents], 20))
            ...
            bookmarks = get_bookmarks(user['upath'])

            print("Getting Gifts...")
            gifts = get_gifts(user['upath'])
            ...
            works = [item for sublist in workchunks for item in sublist]
            for work in works:
                #register the work
                ...
                #only write the metadata if its isn't already there
                if workdict['ident'] not in works_sofar:
