...
class Work(object):
    ...
    async def make_request(self, sess=None):
        req = await sess.request(method='GET', url=('https://archiveofourown.org/works/%s?view_adult=true&show_comments=true&view_full_work=true' % self.id))
        ...
        if 'This work is only available to registered users' in await req.text():
            raise RestrictedWork('Looking at work ID %s requires login')

        self._html = await req.text()
        #print("ID: %s" % self.id)
