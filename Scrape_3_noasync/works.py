...
class Work(object):
    ...
    def make_request(self, req_delay=5):
        req = request_wrapper(('https://archiveofourown.org/works/%s?view_adult=true&show_comments=true&view_full_work=true' % self.id), req_delay)
        ...
        self._html = req.content
        #print("ID: %s" % self.id)
        #print("Length: %s" % len(self._html))
        self._soup = BeautifulSoup(self._html, 'html.parser')
        if re.search(r'Retry later', self._soup.text) and len(self._soup.text) < 1000:
            raise requests.exceptions.ConnectionError

        if not self._soup.find('dl', attrs={'class': 'work meta group'}):
            raise RestrictedWork('Work %r is sending us to a strange page...' % self.id)
